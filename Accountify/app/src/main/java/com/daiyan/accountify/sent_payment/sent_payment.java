package com.daiyan.accountify.sent_payment;
...
public class sent_payment extends AppCompatActivity {
    ...
    TextInputEditText person_name, payment_amount;
    Cursor cursorLoad;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_sent_payment);
    ...
    public void onclickListener(){
        ...
        save.setOnClickListener(v -> {
            ...
            if(person_name.getText().toString().trim().isEmpty() || payment_amount.getText().toString().trim().isEmpty()){
...
//        });
    ...
    public void save_data(){
        ...
        String amountStr = payment_amount.getText().toString().trim();
        ...
        // Sabse purane Purchase bills uthao
        Cursor cursor = db.rawQuery("SELECT id, pending, payment_amount FROM " + db_connect_obj.getTable_name_product() + " WHERE person_name = ? AND trade_type = 'purchase' AND CAST(pending AS REAL) > 0 ORDER BY id ASC", new String[]{name});
        ...
        if (paymentToApply > 0) {
            Cursor lastBill = db.rawQuery("SELECT id, pending, payment_amount FROM " + db_connect_obj.getTable_name_product() + " WHERE person_name = ? AND trade_type = 'purchase' ORDER BY id DESC LIMIT 1", new String[]{name});
        ...
        Toast.makeText(this, "Payment Sent & Handled by Database!", Toast.LENGTH_SHORT).show();
    }
