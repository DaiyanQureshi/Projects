...
public class trade_sale extends AppCompatActivity {
    ...
    TextView save, save_and_new, billed_item_arrow_text, add_item_text, total_amount, balance_due_rupee, pending;
    ...
    private void calculate_pending() {
        // safeParseBigDecimal crash hone se bachayega
        BigDecimal total = safeParseBigDecimal(total_amount.getText().toString());
        BigDecimal received = safeParseBigDecimal(payment_amount.getText().toString());

        // Asli logic: Total mein se Received ghataya
        BigDecimal due = total.subtract(received);

        // UI par set kiya
        pending.setText(due.toString());

        // Color logic: Agar paise lene hain toh Red, agar barabar h toh Gray
        int color = (due.compareTo(BigDecimal.ZERO) > 0) ? Color.parseColor("#AB0836") :
                (due.compareTo(BigDecimal.ZERO) == 0) ? Color.parseColor("#7F8081") : Color.parseColor("#08BD7C");
    ...
    public void onClickListener() {
        ...
        save.setOnClickListener(v -> { if(save_data()) finish(); });
    ...
    public void set_list_item() {
        String[] columns = {"_id", "item_name", "quantity", "rate", "total_amount", "subtotal", "payment_amount"};
        MatrixCursor cursor = new MatrixCursor(columns);
        int id = 1;
        for (ContentValues cv : product_data) {
            cursor.addRow(new Object[]{id++, cv.getAsString("item_name"), cv.getAsString("quantity"), cv.getAsString("rate"), cv.getAsString("total_amount"), cv.getAsString("subtotal"), safeString(cv.getAsString("payment_amount"))});
        }
    ...
    private String safeString(String text) {
        return (text == null || text.trim().isEmpty()) ? "0" : text;
    }
